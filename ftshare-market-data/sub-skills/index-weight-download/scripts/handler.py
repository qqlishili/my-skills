#!/usr/bin/env python3
"""根据 url_hash 下载指数权重 Excel（xlsx，market.ft.tech）"""
import argparse
import json
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description="下载指数权重 xlsx（路径参数 url_hash）")
    parser.add_argument(
        "--url-hash",
        required=True,
        help="index-weight-list 或 index-weight-summary 返回的 url_hash",
    )
    parser.add_argument(
        "--output",
        default=None,
        help="保存路径（默认当前目录下 {url_hash}.xlsx）",
    )
