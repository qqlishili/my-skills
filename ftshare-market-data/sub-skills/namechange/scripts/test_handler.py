...
class TestFetch(unittest.TestCase):
    ...
    def test_fetch_by_trade_code(self, mock_open):
        ...
        self.assertIn("/api/v1/market/data/namechange", url)
        self.assertIn("trade_code=600848.SH", url)

    @patch.object(handler, "safe_urlopen")
    def test_fetch_with_date_range(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.return_value = b"[]"
        handler.fetch("600848.SH", "20200101", "20241231")
        url = mock_open.call_args[0][0]
        self.assertIn("start_date=20200101", url)
