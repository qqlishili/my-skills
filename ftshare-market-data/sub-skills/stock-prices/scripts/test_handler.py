#!/usr/bin/env python3
"""Tests for stock-prices handler"""
import json
import sys
import unittest
import urllib.error
...
class TestFetch(unittest.TestCase):
    ...
    def test_url_uses_daec_endpoint_with_symbol_and_range(self, mock_open):
        ...
        handler.fetch("600000.XSHG", "TODAY", None)
    ...
    def test_url_with_trade_days_ago(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.return_value = b"[]"
        handler.fetch("600000.XSHG", "TRADE_DAYS_AGO(5)", None)
        url = mock_open.call_args[0][0].full_url
    ...
    def test_url_with_ts_ms(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.return_value = b"[]"
        handler.fetch("600000.XSHG", None, 1735689600000)
        url = mock_open.call_args[0][0].full_url
    ...
    def test_wraps_bare_array_and_converts_ts_ms(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.return_value = (
            b'[{"ts_ms":1782092100000,"price":9.05,"avg_price":9.03,"volume":400700,"turnover":3618333.0}]'
        )
        result = handler.fetch("600000.XSHG", "TODAY", None)
        self.assertEqual(len(result["prices"]), 1)
        rec = result["prices"][0]
        self.assertEqual(rec["price"], 9.05)
        # ts_ms 被转成 ISO 字符串，不再是整数
    ...
    def test_retries_on_incomplete_read(self, mock_open):
        ...
        mock_open.return_value.__enter__.return_value.read.side_effect = [
            ...
            b'[{"ts_ms":1782092100000,"price":9.05}]',
        ]
        result = handler.fetch("600000.XSHG", "TODAY", None)
        self.assertEqual(mock_open.call_count, 2)
        self.assertEqual(result["prices"][0]["price"], 9.05)
...
class TestMain(unittest.TestCase):
    ...
    def test_main_since_today(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.return_value = b"[]"
        with patch.object(sys, "argv", ["handler.py", "--stock", "600000.XSHG", "--since", "TODAY"]):
