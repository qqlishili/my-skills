...
class TestBuildParams(unittest.TestCase):
    ...
    def test_basic(self):
        p = handler.build_params("000001.XSHG", "20240101", "20240131")
        self.assertEqual(p["symbol"], "000001.XSHG")
    ...
    def test_interval_week(self):
        p = handler.build_params("000001.XSHG", "20240101", "20240601", interval="Week")
        self.assertEqual(p["interval"], "Week")

    def test_adjust_forward(self):
        p = handler.build_params("000001.XSHG", "20240101", "20240131", adjust="Forward")
    ...
    def test_bad_since_raises(self):
        with self.assertRaises(ValueError):
            handler.build_params("000001.XSHG", "2024-01-01", "20240131")

    def test_bad_until_raises(self):
        with self.assertRaises(ValueError):
            handler.build_params("000001.XSHG", "20240101", "2024131")

    def test_short_date_raises(self):
        with self.assertRaises(ValueError):
            handler.build_params("000001.XSHG", "202401", "20240131")
...
class TestFetch(unittest.TestCase):
    ...
    def test_url_uses_daec_endpoint(self, mock_open):
        ...
        handler.fetch("000001.XSHG", "20240101", "20240131")
    ...
    def test_url_with_adjust(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.return_value = b"[]"
        handler.fetch("000001.XSHG", "20240101", "20240131", adjust="Forward")
        url = mock_open.call_args[0][0].full_url
    ...
    def test_wraps_bare_array_and_converts_ts(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.return_value = (
            b'[{"open_ts_ms":1717378200000,"close_ts_ms":1717398000000,"open":"4067.1578","high":"4093.0405","low":"4045.6898","close":"4057.74","volume":67602580600,"turnover":"1319801913950.9"}]'
        )
        result = handler.fetch("000001.XSHG", "20240101", "20240131")
        self.assertEqual(len(result["ohlcs"]), 1)
        rec = result["ohlcs"][0]
        self.assertEqual(rec["close"], "4057.74")
    ...
    def test_retries_on_incomplete_read(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.side_effect = [
            ...
            b'[{"open_ts_ms":1717378200000,"close":"4057.74"}]',
        ]
        result = handler.fetch("000001.XSHG", "20240101", "20240131")
        self.assertEqual(mock_open.call_count, 2)
        self.assertEqual(result["ohlcs"][0]["close"], "4057.74")
...
class TestMain(unittest.TestCase):
    ...
    def test_main_explicit_until(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.return_value = b"[]"
        with patch.object(sys, "argv", ["handler.py", "--index", "000001.XSHG", "--since", "20240101", "--until", "20240131"]):
    ...
    def test_main_default_until_today(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.return_value = b"[]"
        with patch.object(sys, "argv", ["handler.py", "--index", "000001.XSHG", "--since", "20240101"]):
