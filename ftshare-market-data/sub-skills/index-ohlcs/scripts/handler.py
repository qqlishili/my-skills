#!/usr/bin/env python3
"""查询单只指数 OHLC K 线（daec history/ohlcs，日期区间）"""
import argparse
import json
import re
import sys
...
def fetch(index: str, since: str, until: str, interval: str = "Day", adjust: Optional[str] = None) -> dict:
    params = build_params(index, since, until, interval, adjust)
...
def main():
    parser = argparse.ArgumentParser(description="查询单只指数 OHLC K 线（daec，日期区间）")
    parser.add_argument(
        "--index",
        required=True,
    ...
    try:
        result = fetch(args.index, args.since, until, args.interval, args.adjust)
