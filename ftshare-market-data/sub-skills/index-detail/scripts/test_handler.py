#!/usr/bin/env python3
"""Tests for index-detail handler"""
import json
import sys
import unittest
import urllib.error
...
class TestBuildParams(unittest.TestCase):
    ...
    def test_symbol_only(self):
        self.assertEqual(handler.build_params("000001.XSHG"), {"symbol": "000001.XSHG"})

    def test_with_masks(self):
        p = handler.build_params("000001.XSHG", "name,close")
        self.assertEqual(p, {"symbol": "000001.XSHG", "masks": "name,close"})
...
class TestFetch(unittest.TestCase):
    ...
    def test_url_uses_daec_endpoint_with_symbol(self, mock_open):
        ...
        handler.fetch("000001.XSHG")
        url = mock_open.call_args[0][0].full_url
        self.assertIn("daec/index", url)
    ...
    def test_url_with_masks(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.return_value = b"{}"
        handler.fetch("000001.XSHG", "name,close")
        url = mock_open.call_args[0][0].full_url
    ...
    def test_converts_ts_millis_to_iso(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.return_value = (
            b'{"symbol":"000001.XSHG","close":"4057.74","pe_ttm":17.33,"ts_millis":1780300786000}'
        )
        result = handler.fetch("000001.XSHG")
        self.assertEqual(result["close"], "4057.74")
    ...
    def test_retries_on_incomplete_read(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.side_effect = [
            ...
            b'{"symbol":"000001.XSHG","ts_millis":1780300786000}',
        ]
        result = handler.fetch("000001.XSHG")
        self.assertEqual(mock_open.call_count, 2)
        self.assertEqual(result["symbol"], "000001.XSHG")
...
class TestMain(unittest.TestCase):
    ...
    def test_main_basic(self, mock_open):
        mock_open.return_value.__enter__.return_value.read.return_value = b'{"symbol":"000001.XSHG"}'
