...
def fetch_page(page: int, page_size: int, **extra) -> dict:
    ...
    try:
        ...
        print("提示：v1 错误信息被统一吞掉；排查根因可打 v0：/api/v0/us/us-cashflow", file=sys.stderr)
        sys.exit(1)


def main():
    parser = argparse.ArgumentParser(description="查询美股现金流量表（长表 EAV，按财年 + 报告期）")
