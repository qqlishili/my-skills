...
description: 批量获取多只可转债 K 线 POST 接口（market.ft.tech，convertible-bond-candlesticks/batch）。用户问多只可转债的 K 线、批量可转债日 K/周 K/月 K/年 K 时使用。必填 --symbols、--interval-unit、--until-ts-millis；可选 --interval-value、--adjust-kind、--since-ts-millis、--limit。
---

# 批量可转债 K 线 - 批量查询多只可转债 K 线（convertible-bond-candlesticks/batch）
