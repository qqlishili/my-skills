#!/usr/bin/env python3
"""查询港股利润表（一般企业/银行/保险），宽表，按股票代码/年度+报告类型/日期范围查询"""
import argparse
import json
import sys
import urllib.error
...
ENDPOINTS = {
    "gene": "/api/v1/market/data/hk/hk-income-gene",
    "bank": "/api/v1/market/data/hk/hk-income-bank",
    "insur": "/api/v1/market/data/hk/hk-income-insur",
}
...
def main():
    parser = argparse.ArgumentParser(description="查询港股利润表（一般企业/银行/保险）")
    ...
    if report_type is not None and report_type not in VALID_REPORT_TYPES:
        ...
        sys.exit(1)
    # year 与 report_type 必须同时出现或同时不出现
