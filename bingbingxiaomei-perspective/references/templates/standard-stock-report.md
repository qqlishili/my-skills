...
> **核心原则**：先按当前 taxonomy 分流，再进入八步框架。不分流会退回通用财务报告。
...
> 个股分析必须先判断问题结构，再选择当前模型；模型解释问题，启发式约束研究和决策动作。

### 分流表

| 分流问题 | 当前模型 |
|---|---|
| 是国家战略、产业政策、国产替代或出海竞争吗？ | `m03` 国家目标到产业映射模型 |
| 要验证技术壁垒、订单、产能、利润或产业链地位吗？ | `m04` 产业兑现链模型 |
| 是宏观冲击、美元、美债、石油或地缘风险吗？ | `m01` 三要素状态模型 + `m02` 风险与时间窗口模型 |
| 是公告、小作文、题材传播或来源真实性问题吗？ | `m05` 证据状态模型 |
| 是仓位、杠杆、回撤承受力或退出问题吗？ | `m06` 风险预算与自我约束模型 |
| 都不是 | 从 `m01` 开始，并说明为什么其他模型不适用 |

### 分流输出（1-2 句）

> 当前模型：{{m01-m06}}。启发式：{{h01-h09，可为空}}。理由：{{一句话说明选择依据}}。

- [ ] 已完成分流表判断
- [ ] 已输出当前模型、启发式与理由（1-2 句）
...
> `m01` 只描述三要素是否共振；实际风险暴露还须由 `m06` 与 `h02` 约束，不能机械套固定仓位。
...
3. **taxonomy 分流强制**：第 0.5 节必须在八步框架之前完成，ID 只读取 `references/taxonomy.json`
...
*模板版本：v1.0（标准版）*
*基于：冰冰小美 F.看个股逻辑八步框架 + 当前 taxonomy（`m01-m06` / `h01-h09`）*
