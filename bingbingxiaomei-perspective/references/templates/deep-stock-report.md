...
> **taxonomy**：模型与启发式 ID 只从 [references/taxonomy.json](../taxonomy.json) 读取。
...
> 深度版必须先判断问题结构，并把每个当前模型落到报告对应章节。模型解释问题，启发式约束研究和决策动作。

### 分流表

| 分流问题 | 当前模型 | 报告落点 |
|---|---|---|
| 是国家战略、产业政策、国产替代或出海竞争吗？ | `m03` 国家目标到产业映射模型 | §2.2 时代主线校验 + §3.1 行业基本面 |
| 要验证壁垒、订单、产能、利润或产业链地位吗？ | `m04` 产业兑现链模型 | §1.2 技术与产业验证 + §3.1 行业基本面 |
| 是宏观冲击、美元、美债、石油或地缘风险吗？ | `m01` 三要素状态模型 + `m02` 风险与时间窗口模型 | §3.2 三要素扫描 + §7 风险分析 |
| 是公告、小作文、题材传播或来源真实性问题吗？ | `m05` 证据状态模型 | §5.1 信息源 + §5.2 假象识别 |
| 是仓位、杠杆、回撤承受力或退出问题吗？ | `m06` 风险预算与自我约束模型 | §7 风险分析 + §8 投资结论 |
| 以上都涉及 | 按 taxonomy precedence 组合 | 按顺序说明主模型与否决条件 |
| 都不是 | 从 `m01` 开始 | §3.2 三要素扫描，并说明其他模型为何不适用 |

### 当前模型-报告章节落点对照

| 当前模型 | 报告落点 | 深度版要求 |
|---|---|---|
| `m01` 三要素状态模型 | §3.2 行业与竞争格局、§5 市场预期 | 分别判断方向、流动性与情绪，不把单项有利写成整体有利 |
| `m02` 风险与时间窗口模型 | §7 风险分析、§8 投资结论 | 区分事件前等待、风险减弱与落地验证，不做日期预测 |
| `m03` 国家目标到产业映射模型 | §2.2 时代主线校验、§3.1 产业政策 | 国家战略只生成研究候选，不直接推出个股结论 |
| `m04` 产业兑现链模型 | §1.2 技术验证、§3.1 行业基本面 | 核验壁垒、订单、产能、利润与估值，允许否决战略映射 |
| `m05` 证据状态模型 | §5.1 来源、§5.2 假象识别 | 原文、财报和交叉印证决定证据状态，权威来源不等于结论成立 |
| `m06` 风险预算与自我约束模型 | §7 风险分析、§8 投资结论 | 用仓位、现金余量与承受力约束暴露，不用固定比例套所有用户 |

### 分流输出（2-3 句）

> 主模型：{{m01-m06}}。辅助模型：{{m01-m06，可为空}}。启发式：{{h01-h09，可为空}}。
> 理由：{{一句话说明选择依据，以及它如何决定后续分析重点}}。
> 报告落点：{{列出本次分析将重点展开的章节}}。

- [ ] 已完成分流表判断（逐行检查）
- [ ] 已确定主模型、辅助模型与启发式
- [ ] 已输出当前 ID、理由与报告落点（2-3 句）
- [ ] 已确认各模型对应的报告章节将重点展开
...
> `m04` + `h05` 提醒：标签不能替代壁垒、订单、产能和盈利验证。
...
> 资产标签不是结论：用 `m04` 与 `h05` 核验增长、现金流、订单、盈利和估值是否兑现。
...
> `m01` 只描述三要素是否共振；风险暴露由 `m06` 与 `h02` 结合用户承受力决定，不机械套固定仓位。
...
> 用 `m04` 验证自由现金流、订单和盈利兑现，再用 `m06` 判断该载体是否适合当前风险预算；历史经验不能直接升级为固定避风港规则。
...
- [ ] 美联储路径变化后，`m01` / `m02` 的流动性与时间窗口判断是否失效？
- [ ] 新登/老登标签被市场重新混合后，`m04` 的兑现证据是否仍成立？
- [ ] 流动性挤压从分化转为全面下行后，`m01` 的三要素状态是否改变？
- [ ] 中美货币政策重新同步后，`m02` 的风险窗口是否需要重估？
- [ ] 任一触发→回退到基础三要素做交叉验证
...
- [ ] 加密货币/外汇：不适用
- [ ] 非科技行业：不得机械套用 `m03` 的国家目标映射，仍须由 `m04` 验证兑现
- [ ] 技术面与基本面冲突时：按 `h05` 验证壁垒、订单与盈利，并按 `h09` 判断载体是否失效
...
*模板版本：v3.0*
*基于：冰冰小美 F.看个股逻辑八步框架 + 当前 taxonomy（`m01-m06` / `h01-h09`）+ 7轮实测验证*
